from DataProcess import prepare_dataset, cut_dataset
...
class GetDataLoader(object):
    ...
    def get_dataset(self, train_transform, test_transform):
        ...
        elif self.opt.dataset == "Tiny-ImageNet-200":
            train_dataset = TinyImageNet(root=self.data_path, train=True, transform=train_transform)
            test_dataset = TinyImageNet(root=self.data_path, train=False, transform=test_transform)
            dataset = train_dataset
    ...
    def get_STL_pretrain(self):
        ...
        unlabel_dataset = datasets.STL10(self.data_path,
                                         ...
                                         transform=train_transform,
                                         download=False,)
        unlabel_dataset1 = cut_dataset(unlabel_dataset, 10000)
        target_train_loader = torch.utils.data.DataLoader(
            unlabel_dataset1, batch_size=self.opt.batch_size, shuffle=True, drop_last=True, num_workers=self.opt.num_workers)
    ...
    def get_data_unsupervised(self):
        ...
        # target_train, target_test, shadow_train, shadow_test = prepare_dataset(
        #     dataset)
        target_train = cut_dataset(dataset, 10000)
        target_test = cut_dataset(dataset, 10000)
        shadow_train = cut_dataset(dataset, 10000)
        shadow_test = cut_dataset(dataset, 10000)
        print("Preparing dataloader!")
        target_train_loader = torch.utils.data.DataLoader(
    ...
    #     target_train,
    #     num_workers=0,
    #     batch_sampler=UniformWithReplacementSampler(num_samples=len(target_train), sample_rate=64/10000)
    # )
        target_test_loader = torch.utils.data.DataLoader(
